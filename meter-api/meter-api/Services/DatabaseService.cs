...
public class DatabaseService(IDatabaseHttpClient databaseClient, Database database, IOptions<DatabaseOptions> options, SemaphoreSlim semaphoreSlim) : IDatabaseService
{
    ...
    public async Task<T> Create<T>(T entity, bool needsLock = true) where T : class, IDatabaseObject
    {
        if (needsLock)
        {
            await semaphoreSlim.WaitAsync();
    ...
    public async Task<T> Put<T>(T entity, bool needsLock = true) where T : class, IDatabaseObject
    {
        if (needsLock)
        {
            await semaphoreSlim.WaitAsync();
