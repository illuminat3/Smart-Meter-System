  ...
  <div class="min-h-screen">
    <div class="flex items-center justify-between p-4">
      <div class="text-sm text-gray-500">
        Status: <span :class="connectionStatusClass">{{ connectionStatus }}</span>
      </div>
      <LogoutComponent />
    </div>

    <div class="p-4">
      <div v-if="meterList.length === 0" class="text-center text-gray-500 py-16">
      ...
      <div v-else class="grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        <Card v-for="m in meterList" :key="m.meterId" class="shadow-sm">
          ...
          <template #footer>
            <div class="text-xs text-gray-500 text-right">Last updated: {{ fmtTime(m.lastUpdated) }}</div>
          </template>
        </Card>
      </div>
    </div>
