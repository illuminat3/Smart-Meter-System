import {connect, disconnect, off as offEvent, on as onEvent, useSignalRStatus} from "@/lib/signalr";
...
export function useMeters() {
    ...
    function normalizeMeter(x: any): Meter | undefined {
        ...
        const displayName: string = String(x.displayName ?? x.DisplayName ?? x.name ?? x.Name ?? meterId);
        const currentUsage: number = Number(x.currentUsage ?? x.CurrentUsage ?? x.current ?? x.value ?? 0);
    ...
    function upsertMeters(list: Meter[]) {
        ...
        for (const m of list) {
            ...
            const merged = {
                ...(prev || { meterId: m.meterId, displayName: m.displayName, currentUsage: 0, totalUsage: 0, totalCost: 0 }),
                ...m,
            } as Meter;
            if (!prev ||
